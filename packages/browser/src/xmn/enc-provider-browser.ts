...
export class EncryptionBrowser {
  ...
  private async encryptSymKey() {
    ...
    if (!this.ci.useEncryption) return new ArrayBuffer(0)
  ...
  private async encrypt(ar: Uint8Array): Promise<ArrayBuffer> {
    return this.ci.useEncryption ? await crypto.subtle.encrypt(SYM_ALGO, this.ci.syncKey, ar) : this.getArrayBuffer(ar)
  }

  private async decrypt(ar: Uint8Array): Promise<ArrayBuffer> {
    return this.ci.useEncryption ? await crypto.subtle.decrypt(SYM_ALGO, this.ci.syncKey, ar) : this.getArrayBuffer(ar)
  }
  ...
  public async setNewKey(rc, syncKey: string) {

    rc.isAssert() && rc.assert(rc.getName(this), this.ci.useEncryption)
  ...
  private async ensureSyncKey() {
    ...
    if (!this.ci.useEncryption) return
