...
export class EncryptionBrowser {
  ...
  async encodeHeader(rc: RunContextBrowser) {
    ...
    const encKey = await this.encryptSymKey(),
          obj    = {
            ...
            location    : this.ci.customData.location,
            now         : Date.now()
          }
  ...
  private async encryptSymKey() {
    ...
    if (!this.si.useEncryption) return new ArrayBuffer(0)
    ...
    */
   const buffer = await crypto.subtle.exportKey('raw', this.si.syncKey),
  ...
  private async encrypt(ar: Uint8Array): Promise<ArrayBuffer> {
    return this.si.useEncryption ? await crypto.subtle.encrypt(SYM_ALGO, this.si.syncKey, ar) : this.getArrayBuffer(ar)
  }

  private async decrypt(ar: Uint8Array): Promise<ArrayBuffer> {
    return this.si.useEncryption ? await crypto.subtle.decrypt(SYM_ALGO, this.si.syncKey, ar) : this.getArrayBuffer(ar)
  }
  ...
  async encodeBody(rc: RunContextBrowser, data: WireObject[]): Promise<Uint8Array> {
    ...
    rc.isDebug() && rc.debug(rc.getName(this), 'encodeBody', {
      ...
      encrypted   : this.si.useEncryption,
