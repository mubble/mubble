...
export class XmnRegistry {
  ...
  static enrollApi(api: string, parent: any, perm: PERM) {

    if (this.register[api]) {
      const msg = `Duplicate definition for xmn api/event found: ${api}`
      console.error(msg)
      throw(Error(msg))
    }

    // console.log('enrolled api', api)
    this.register[api] = {name: api, isApi: true, parent, perm}
  }
  ...
  static commitRegister(rc: RunContextBase, router: XmnRouter, providers: any[]) {
  ...
  private static checkForProvider(rc: RunContextBase, router: XmnRouter, provider: any): boolean {
